...
// tokens and emit kubectl ExecCredentials. It is stored in a viper config
// file under `auth.<sub>:<clusterID>`.
...
// `dev.podplane.auth.<sub>:<clusterID>`.
...
// secrets. The full key is `<keyringPrefix><sub>:<clusterID>`.
const keyringPrefix = "dev.podplane.auth."

func authKey(sub, clusterID string) string {
...
func (c *Config) AuthSet(meta AuthMetadata, secrets AuthSecrets) error {
	...
	key := authKey(meta.Sub, meta.ClusterID)

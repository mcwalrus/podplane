...
func (m *Local) WaitForSystemServices(ctx context.Context, opts WaitOptions) error {
	...
	for {
		...
		remaining := time.Until(deadline)
		if remaining <= 0 {
			status, _ := m.vm.Shell(ctx, "systemctl --no-pager --plain is-active "+services, sshPort, vm.ShellOptions{Timeout: 5 * time.Second})
		...
		_, err := m.vm.Shell(ctx, "systemctl is-active --quiet "+services, sshPort, vm.ShellOptions{Timeout: commandTimeout})
		if err == nil {
			if !opts.Quiet {
...
func (m *Local) WaitForNetsyHealth(ctx context.Context, opts WaitOptions) error {
	...
	for {
		...
		remaining := time.Until(deadline)
		if remaining <= 0 {
			health, _ := m.vm.Shell(ctx, "curl -fsS --max-time 5 http://127.0.0.1:8080/health || true", sshPort, vm.ShellOptions{Timeout: 10 * time.Second})
		...
		_, err := m.vm.Shell(ctx, "curl -fsS --max-time 5 http://127.0.0.1:8080/health", sshPort, vm.ShellOptions{Timeout: commandTimeout})
		if err == nil {
			if !opts.Quiet {
...
func (m *Local) ProbeCloudInit(ctx context.Context) (string, error) {
	...
	output, err := m.vm.Shell(ctx, "cloud-init status", sshPort, vm.ShellOptions{Timeout: 5 * time.Second})
	trimmed := strings.TrimSpace(string(output))
	if err != nil && trimmed == "" {
		return "", err
	}

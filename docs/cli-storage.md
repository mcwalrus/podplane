...
- **Auth metadata file**: stores durable metadata needed to resolve auth state, but not token secrets. It influences CLI/kubectl auth behavior and is small, structured, persistent configuration.
...
- **Dependencies cache**: dependency files are grouped by dependency domain under the dependencies cache root. VMConfig manifests and artifacts live under `vmconfig/`, while component image metadata and blobs for the local zot storage live under `components/`.
